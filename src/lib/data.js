export const answers = [
  "ALBERT EINSTEIN: The Smiley did not cross the road. The road passed beneath the Smiley.",

  "ISAAC NEWTON: Smileys at rest tend to stay at rest. Smileys in motion tend to cross roads.",

  "41st. President GEORGE W. BUSH [the Father]: Read my lips; 'No smiley crossed any road'",

  "43rd. President GEORGE W. BUSH [the Son]: We don't really care why the smiley crossed the road. We just want to know if the smiley is on our side of the road or not. The smiley is either with us or it is against us. There is no middle ground here.",

  "MOHAMMED ALDOURI: (Iraq's ambassador) The smiley did not cross the road. This is a complete fabrication. We don't even have a smiley.",

  "JERRY FALWELL: Because the smiley was gay! Isn't it obvious? Can't you people see the plain truth in front of your face? The smiley was going to the 'other side.' That's what they call it -- the other side. Yes, my friends, that smiley is gay. And, if you eat that smiley, you will become gay too.",

  "MARTIN LUTHER KING, JR.: I envision a world where all smileys will be free to cross roads without having their motives called into question.",

  "GRANDPA: In my day, we didn't ask why the smiley crossed the road. Someone told us that the smiley crossed the road, and that was good enough for us.",

  "JOHN LENNON: Imagine all the smileys crossing roads in peace.",

  "ARISTOTLE: It is the nature of smileys to cross the road.",

  "KARL MARX: It was an historical inevitability.",

  "CAPTAIN KIRK: To boldly go where no smiley has gone before.",

  "SIGMUND FREUD: The fact that you are at all concerned that the smiley crossed the road reveals your underlying sexual insecurity.",

  "ALBERT EINSTEIN: Did the smiley really cross the road or did the road move beneath the smiley?",

  "BILL CLINTON: I did not cross the road with THAT smiley. What do you mean by smiley? Could you define the word 'smiley', please?",

  "THE BIBLE: And God came down from the heavens, and He said unto the smiley, 'Thou shalt cross the road.' And the smiley crossed the road, and there was much rejoicing.",

  "It didn't. Chuck Norris threw it.",

  "BUDDHA: If you meet the smiley on the road, kill it.",

  "CHARLES DARWIN: It was the logical next step after coming into this world.",

  "EPICURUS: For fun.",

  "NIETZSCHE: Because if you gaze too long across the Road, the Road gazes also across you.",

  "THE SPHINX: You tell me.",
];
